...
/**
 * http请求类<br>
 * Http请求类用于构建Http请求并同步获取结果，此类通过 {@link CookiePool}
 * 持有域名对应的Cookie值，再次请求时会自动附带Cookie信息
 * 
...
public class HttpRequest extends HttpBase<HttpRequest> {
	 ...
	 * 
	 * @param cookies
	 *            Cookie值数组，如果为{@code null}则设置无效，使用默认Cookie行为
	 * @return this
	 * @since 3.1.1
	 */
	 ...
	 * 
	 * @param cookie
	 *            Cookie值，如果为{@code null}则设置无效，使用默认Cookie行为
	 * @return this
	 * @since 3.0.7
	 */
	 ...
	 *            名
	 * @param dataSource
	 *            数据源，文件可以使用{@link FileDataSource}包装使用
	 * @return this
	 * @since 4.0.9
	 */
	...
	private void initConnecton() {
		...
		// 初始化 connection
		this.httpConnection = HttpConnection.create(this.url, this.method, this.hostnameVerifier, this.ssf, this.timeout,
				this.proxy).header(this.headers, true); // 覆盖默认Header
	...
	private void urlWithParamIfGet() {
		...
		if (Method.GET.equals(method)) {
			...
			if (ArrayUtil.isNotEmpty(this.bodyBytes)) {
				this.url = HttpUtil.urlWithForm(this.url, StrUtil.str(this.bodyBytes, this.charset), this.charset,
						this.isEncodeUrl);
			} else {
	...
	private boolean isIgnoreResponseBody() {
		...
		if (Method.HEAD == this.method || Method.CONNECT == this.method || Method.OPTIONS == this.method
