...
import top.coos.core.io.resource.NoResourceException;
...
public abstract class DSFactory {
	 ...
	 *            数据库连接池实现类，用于检测所提供的DataSource类是否存在，
	 *            当传入的DataSource类不存在时抛出ClassNotFoundException<br>
	 *            此参数的作用是在detectDSFactory方法自动检测所用连接池时，如果实现类不存在，调用此方法会自动抛出异常，
	 *            从而切换到下一种连接池的检测。
	 * @param setting
	 *            数据库连接配置
	 */
		...
		if (null == setting) {
			try {
				...
				try {
					...
					throw new NoResourceException("Default db setting [{}] or [{}] in classpath not found !",
							DEFAULT_DB_SETTING_PATH, DEFAULT_DB_SETTING_PATH2);
				}
			}
		}
