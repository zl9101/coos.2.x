...
import top.coos.db.handler.EntityListHandler;
import top.coos.db.handler.NumberHandler;
import top.coos.db.handler.PageResultHandler;
import top.coos.db.handler.RsHandler;
...
public class SqlConnRunner {
	 ...
	 *             SQL执行异常
	 */
	public List<Entity> findLike(Connection conn, String tableName, String field, String value, LikeType likeType)
			throws SQLException {
	...
	public PageResult<Entity> page(Connection conn, Collection<String> fields, Entity where, Page page) throws SQLException {
		...
		final int count = count(conn, where);
		PageResultHandler pageResultHandler = PageResultHandler.create(new PageResult<Entity>(page.getPageNumber(), page
				.getPageSize(), count));
	 ...
	 * 
	 * @param wrapperChar
	 *            包装字符，字符会在SQL生成时位于表名和字段名两边，null时表示取消包装
	 * @return this
	 * @since 4.0.0
	 */
