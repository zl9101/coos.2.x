...
import top.coos.core.lang.VoidFunc;
...
public class Session extends AbstractSqlRunner implements Closeable {
	...
	public boolean isSupportTransaction() throws DbRuntimeException {
		...
		if (null == isSupportTransaction) {
			try {
				...
				throw new DbRuntimeException(e, "Because of SQLException [{}], We can not know transation support or not.",
						e.getMessage());
			}
		}
	 ...
	 * 
	 * @param func
	 *            函数抽象，在函数中执行多个SQL操作，多个操作会被合并为同一事务
	 * @since 3.2.3
	 */
